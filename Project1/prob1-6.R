...
edgelistFile <- "~/Dropbox/232e hw/proj1/facebook_combined.txt"
...
plot(d, main="curve fit of Facebook Graph", xlab="Nodes", ylab="Degree Distribution")
x <- 1:length(d)
fit <- lm(d~poly(1/x,3))
curvefit <- predict(fit)
lines(x, curvefit, col="red", lwd=2)
mean(degree(g))

sum <- 0
for (i in 1:length(d)){
  sum <- sum + (curvefit[[i]]-d[[i]])^2
}
mse <- sum/length(d)

#############     PROBLEM 2    ##############
subGraphNodes <- neighborhood(g, 1, nodes=V(g)[1])
